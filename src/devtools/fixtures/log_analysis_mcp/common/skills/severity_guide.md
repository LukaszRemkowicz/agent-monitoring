...
- **CRITICAL**: Confirmed service-affecting failure or successful exploitation.
  Examples include repeated upstream 5xx with user impact, DB/Redis unreachable,
  Celery tasks exhausting retries, an unexpected non-empty 2xx response on a
  sensitive path, or confirmed malicious-input impact. Access logs alone do not
  confirm the response contents or exfiltration.
...
- Do not classify a Traefik 5xx as an application outage when proxy analysis
  reports no upstream attempt. Correlate it with CrowdSec/AppSec evidence and
  treat a confirmed edge block as a security event.

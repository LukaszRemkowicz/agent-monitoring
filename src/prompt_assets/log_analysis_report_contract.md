...
- evidence: current MCP tool results when present, otherwise collected snapshot metadata, previous_analysis, mandatory system-prompt skills, or historical context used to produce the report.
- coverage_gaps: Sources or checks that were unavailable or inconclusive.
- recommendations: Concrete next steps; when evidence shows expected scanner/probe noise without service impact, say no immediate routing, application, or mitigation-control change is indicated instead of inventing remediation.

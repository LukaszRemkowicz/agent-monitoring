...
MCP_WORKFLOW_JWT="${MCP_WORKFLOW_JWT:-}"
MCP_KEYCLOAK_URL="${MCP_KEYCLOAK_URL:-}"
MCP_KEYCLOAK_CLIENT_ID="${MCP_KEYCLOAK_CLIENT_ID:-}"
MCP_KEYCLOAK_CLIENT_SECRET="${MCP_KEYCLOAK_CLIENT_SECRET:-}"
if [[ -z "$MCP_WORKFLOW_JWT" ]]; then
    if [[ -z "$MCP_KEYCLOAK_URL" || -z "$MCP_KEYCLOAK_CLIENT_ID" || -z "$MCP_KEYCLOAK_CLIENT_SECRET" ]]; then
        log_error "Set MCP_WORKFLOW_JWT or all MCP_KEYCLOAK_* client credential variables."
        exit 1
    fi
fi
